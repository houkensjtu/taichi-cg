...
class BICGPoissonSolver(CGPoissonSolver):
    ...
    def __init__(self, n, eps, offset, quiet):
        ...
        ti.root.pointer(ti.ij, self.N_tot // 16).dense(ti.ij, 16).place(
            self.r_tld, self.p_hat, self.s, self.t, self.s_hat, self.Ashat)
    ...
    def init(self):
        for i, j in ti.ndrange((self.N_ext, self.N_tot - self.N_ext),
            ...
            xl = (i - self.N_ext) * 2.0 / self.N_tot  # xl, yl, zl = [0,1]
            yl = (j - self.N_ext) * 2.0 / self.N_tot
            self.r[i, j] = ti.sin(2.0 * np.pi * xl) * ti.sin(
                2.0 * np.pi *
                yl)  # r[0] = b - Ax, where x = 0; therefore r[0] = b
            self.r_tld[i,
                       j] = ti.sin(2.0 * np.pi * xl) * ti.sin(2.0 * np.pi * yl)
    ...
    def update_p(self):
        for I in ti.grouped(self.p):
            self.p[I] = self.r[I] + self.beta[None] * (
                self.p[I] - self.omega[None] * self.Ap[I])
    ...
    def update_x(self):
        for I in ti.grouped(self.x):
            self.x[I] += self.alpha[None] * self.p_hat[I] + self.omega[
                None] * self.s_hat[I]
    ...
    def solve(self):
        ...
        for i in range(self.steps):
            ...
            if i == 0:
                ...
                self.beta[None] = (self.rho[None] / self.rho_1[None]) * (
                    self.alpha[None] / self.omega[None])
                self.update_p()
            self.update_phat()
